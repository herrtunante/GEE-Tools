...
|---|---|---|---|---|
| Before | PlanetScope | 5 | 2026-05-27 | 3.7 m |
| Before | Vantor WorldView | 4 | 2021-10 → 2026-02 | 0.35–0.58 m |
| After | PlanetScope | 14 | 2026-08-26, 08-28 | 3.4–3.9 m |
| After | SkySat | 4 | 2026-08-27, 08-31 | 0.64–0.81 m |
| After | Pelican | 3 | 2026-08-27 | 0.55 m |
| After | Vantor WV/Legion | 12 | 2026-08-27 → 09-01 | 0.31–0.59 m |

Both epochs have sub-metre coverage of the corridor: the Vantor baseline
includes a cloud-free 0.35 m collect from 5 Feb 2026, and the newest
Vantor Legion collects (1 Sep) reach 0.4 m. The Vantor release also ships
a raw stereo pair (`stereo/`) for DSM work, which this viewer does not
use; derivatives must keep the CC-BY-NC-4.0 terms.

`js/scenes.js` is generated by `tools/make_scenes.py`, which walks both
live catalogs and emits the scene list **and** the display groups —
labels, colours, GSD ranges and pane stacking are all derived from the
data, so new collections appear with correct dates without touching the
app. Rerun it whenever either dataset gains scenes:

```bash
cd tools && python3 make_scenes.py && python3 make_ndvi_change.py
```

Notes: Vantor COGs are YCbCr-JPEG, which geotiff.js returns unconverted,
so the viewer converts Y/Cb/Cr to RGB per pixel; Pelican's 6-band
pansharpened layout is unverified, so Pelican and Vantor scenes stay
true-colour in the band-combination modes.

### Memory discipline

Attaching all 40+ scenes at once crashes the browser tab — several of the
sub-metre COGs are multi-gigapixel. Two rules keep the working set small:
a scene is attached only while it intersects the current view, and
sub-metre scenes wait until zoom ≥ 13 (the status bar says so rather than
letting them look missing). Adjust via `HIRES_GSD` / `HIRES_MIN_ZOOM` in
`js/app.js`.
